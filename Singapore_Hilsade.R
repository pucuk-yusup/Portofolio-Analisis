...
pop_rast <- terra::rast("https://data.worldpop.org/GIS/Population/Global_2000_2020_Constrained/2020/BSGM/SGP/sgp_ppp_2020_constrained.tif")

# –– LOAD SINGAPORE BOUNDARY
sgp_sf <- geodata::gadm("SGP", level = 0, path = tempdir()) |> st_as_sf()
sgp_vect <- terra::vect(sgp_sf)

# –– DOWNLOAD DEM FOR SINGAPORE
dem <- elevatr::get_elev_raster(locations = sgp_sf, z = 10, clip = "locations") |> terra::rast()
dem_sgp <- terra::crop(dem, sgp_vect) |> terra::mask(sgp_vect)
dem_exagg <- dem_sgp * 1.3
...
p <- ggplot() +
  ...
  # 3. Boundary
  geom_sf(data = sgp_sf, fill = NA, color = "white", linewidth = 0.3) +
  ...
  # 5. Title & theme
  labs(
    title = "Singapore Population Density (2020)",
...
#simpan resolusi tinggi
ggsave(
  "singapore_population_dewa.png",
  plot = p,
